...
export function calculateNatalDateBySolar({
  ...
  // 返回修正后的农历日期信息
  return {
    stemKey: STEM_KEYS[stemIndex], // 天干键值
    branchKey: BRANCH_KEYS[branchIndex], // 地支键值
    year, // 修正后的农历年
    monthIndex: targetLunarMonth.getMonth() - 1, // 修正后的农历月索引（从 0 开始）
    day: targetLunarDay.getDay(), // 修正后的农历日
    hourIndex: wrapIndex(hourIndex), // 修正后的时辰索引
  };
}
...
export function getStemAndBranchByYear(year: number): [number, number] {
  ...
  const stemIndex = wrapIndex((year - 4) % 10, 10); // 天干索引
  const branchIndex = wrapIndex((year - 4) % 12); // 地支索引
